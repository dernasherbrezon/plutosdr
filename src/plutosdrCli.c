...
void plutosdr_print_message(const char *message, int ret) {
	char err_str[1024];
...
int plutosdr_configure_and_run(unsigned long int frequency, unsigned long int sampleRate, float gain, unsigned int bufferSize) {
	...
	if (ctx == NULL) {
		plutosdr_print_message("unable to create context: %s\n", -errno);
		return EXIT_FAILURE;
	}
	...
	if (ret < 0) {
		plutosdr_print_message("unable to set frequency: %s\n", ret);
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	}
	...
	if (ret < 0) {
		plutosdr_print_message("unable to read filter_fir_en attribute: %s\n", ret);
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	}
	...
	if (!filterFirEnabled) {
		...
		ret = plutosdr_set_txrx_fir_enable(dev, true);
		if (ret < 0) {
			plutosdr_print_message("unable to enable fir filter: %s\n", ret);
			iio_context_destroy(ctx);
			return EXIT_FAILURE;
		}
	...
	if (gain > 0.0) {
		...
		if (ret < 0) {
			plutosdr_print_message("unable to switch to manual gain: %s\n", ret);
			iio_context_destroy(ctx);
			return EXIT_FAILURE;
		}
		...
		if (ret < 0) {
			plutosdr_print_message("unable to setup gain: %s\n", ret);
			iio_context_destroy(ctx);
			return EXIT_FAILURE;
		}
		...
		if (ret < 0) {
			plutosdr_print_message("unable to switch to slow_attack gain: %s\n", ret);
			iio_context_destroy(ctx);
			return EXIT_FAILURE;
		}
