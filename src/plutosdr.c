...
#define ERROR_CHECK_PLUTO(y, x)           \
  do {                           \
    int __err_rc = (x);          \
    if (__err_rc < 0) {             \
        plutosdr_print_message(y, __err_rc); \
...
int plutosdr_configure_and_run(unsigned long int frequency, unsigned long int sampleRate, float gain, unsigned int bufferSize) {
  ...
  if (ret < 0) {
    plutosdr_print_message("unable to setup context timeout: %s\n", ret);
    iio_context_destroy(ctx);
    return EXIT_FAILURE;
  }
  ...
  ERROR_CHECK_PLUTO("unable to set frequency: %s\n", iio_channel_attr_write_longlong(altVoltage0, "frequency", (long long) frequency));
  ...
  ERROR_CHECK_PLUTO("unable to read filter_fir_en attribute: %s\n", iio_channel_attr_read_bool(voltage0, "filter_fir_en", &filterFirEnabled));
  ...
  if (!filterFirEnabled) {
    ...
    if (ret < 0) {
      plutosdr_print_message("unable to setup fir filter config: %s\n", ret);
      iio_context_destroy(ctx);
      return EXIT_FAILURE;
    }

    ERROR_CHECK_PLUTO("unable to enable fir filter: %s\n", plutosdr_set_txrx_fir_enable(dev, true));
    fprintf(stderr, "fir filter enabled\n");
  }

  ERROR_CHECK_PLUTO("unable to setup sampling frequency: %s\n", iio_channel_attr_write_longlong(voltage0, "sampling_frequency", 8 * sampleRate));
  ERROR_CHECK_PLUTO("unable to setup rf_bandwidth: %s\n", iio_channel_attr_write_longlong(voltage0, "rf_bandwidth", 8 * sampleRate));

  if (gain > 0.0) {
    ERROR_CHECK_PLUTO("unable to switch to manual gain: %s\n", iio_channel_attr_write(voltage0, "gain_control_mode", "manual"));
    ERROR_CHECK_PLUTO("unable to setup gain: %s\n", iio_channel_attr_write_double(voltage0, "hardwaregain", gain));
    fprintf(stderr, "gain manual: %.2f\n", gain);
  } else {
    ERROR_CHECK_PLUTO("unable to switch to slow_attack gain: %s\n", iio_channel_attr_write(voltage0, "gain_control_mode", "slow_attack"));
    fprintf(stderr, "gain auto: slow_attack\n");
  }

  struct iio_device *inputDevice = plutosdr_find_device_with_input_channels(ctx, "-lpc");
  if (inputDevice == NULL) {
  ...
  for (int i = 0; i < nb_channels; i++) {
    ...
    if (!iio_channel_is_output(ch)) {
      ...
      ERROR_CHECK_PLUTO("unable to set fpga decimation: %s\n", iio_channel_attr_write_longlong(ch, "sampling_frequency", (long long) sampleRate));
    }
  }
  ...
  if (sample_size == 0) {
    ...
    plutosdr_print_message("unable to get sample size: %s\n", errno);
    iio_context_destroy(ctx);
    return EXIT_FAILURE;
  }
